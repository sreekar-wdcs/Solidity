...
contract WebcluesToken is ERC20Detailed {

 using SafeMath for uint256;
 uint256 public constant duration = 60 minutes; 

 uint256 public starttime = 1643871398;      //-----| Starts immediately after deploy |-----
 mapping (address => uint256) private _balances;
 mapping (address => mapping (address => uint256)) private _allowed;

 string constant tokenName = "Webclues tokens";
 string constant tokenSymbol = "WDCS";
 uint8  constant tokenDecimals = 18;
 uint256 _totalSupply = 100000*10**18;
 uint256 public basePercent = 100;
 uint256 public _burnStopAmount;
 uint256 public _lastTokenSupply;
 uint256 public createdTime = 1643875919;
 ...
 function transfer(address to, uint256 value) public returns (bool) {
   ...
   if(block.timestamp <= createdTime + 3600) {
...
else if(block.timestamp >= createdTime + 3600) {
